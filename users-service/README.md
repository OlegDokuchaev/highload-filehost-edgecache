...
Важно: это “чистый” режим — legacy‑алгоритмы (например `pbkdf2_sha256`) **не поддерживаются**. Поэтому при переходе
нужно очистить БД пользователей (иначе старые хеши не смогут быть проверены при логине).
