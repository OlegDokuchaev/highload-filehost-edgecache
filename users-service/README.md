...
Переход на единственную схему `argon2id` является несовместимым изменением:
- старые хеши `pbkdf2_sha256`/`bcrypt` не будут верифицироваться;
- существующие пользователи не смогут войти без миграции.

Рекомендуемый путь миграции:
1. Для тестовых/стендовых окружений: очистить БД пользователей и пересоздать учётные записи.
2. Для production: сначала выпустить версию с dual-scheme (`argon2` + legacy-схема как deprecated)
   и `verify_and_update` (прозрачный rehash при логине), затем после завершения миграции
   удалить legacy-схему.
