...
Кэширующий прокси-сервис для раздачи файлов. При первом запросе файл скачивается с origin-сервера, сохраняется на диск и отдаётся клиенту (MISS). Последующие запросы обслуживаются из локального кэша (HIT). При истечении TTL сервис выполняет conditional GET с `If-None-Match` — если origin отвечает `304`, файл отдаётся из кэша без повторной загрузки (REVALIDATED).
